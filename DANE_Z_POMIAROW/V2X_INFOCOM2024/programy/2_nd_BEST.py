...
def load_data(input_folder):
        ...
        if file_name.endswith(".csv"):
            ...
            if file_number is not None:
                ...
                df = pd.read_csv(
                    ...
                    sep=";",
                    header=None,
                    names=["Index", "Timestamp", "Frequency", "Power"],
                )
    ...
    if combined_data:
        ...
        all_data = pd.DataFrame(
            columns=["Index", "Timestamp", "Frequency", "Power", "FileNumber"]
        )
...
def generate_plots(all_data, output_folder):
    ...
    second_max_values = (
        grouped.apply(lambda x: x.nlargest(2, "Power").iloc[1] if len(x) > 1 else None)
        .dropna()
    ...
    plt.plot(
        second_max_values["FileNumber"],
        second_max_values["Power"],
        marker="o",
        linestyle="--",
        color="blue",
        label="Second Max Power",
    )
    ...
    plt.title("Maksymalne i drugie maksymalne wartości mocy sygnału (wszystkie Index)")
    ...
    plt.close()
    print(
        f"Wykres maksymalnych, drugich maksymalnych i minimalnych wartości zapisany: {output_file}"
    )
