...
class RIS_usb:
    ...
    def __init__(self, port: str, baudrate: int = 115200, set_time: float = 1.0):
        try:
            self.ris = serial.Serial(port, baudrate)
            self.ris_set_time = set_time
        except serial.SerialException:
            print("[SERIAL ERROR] Could not open the specified port.")
            exit()
