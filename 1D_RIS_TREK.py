...
def pattern_loop(freq, RIS_list : list, measure_point : str):
    for pattern in patterns_data:
        ...
        with open(f"V2X_22_11_24_{measure_point}.csv", 'a+') as file:
            current_time = datetime.datetime.now()
            file.write(pattern["ID"]+";"+str(current_time))  # Write information about pattern information
            file.write(";")
            file.close()  # CLose the file
            # RIS_usb.read_pattern() #Inofrmation about pattern set on RIS.
        time.sleep(3)
        analyzer.trace_get(f"V2X_22_11_24_{measure_point}.csv")
