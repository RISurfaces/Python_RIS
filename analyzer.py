...
try:
    with open("config.json") as config_f:
        ...
        except TimeoutError or ConnectionAbortedError or ConnectionRefusedError:
            print(
                "[TIMEOUT ERROR] Check is computer and analyzer is connected to the same local network or did the analyzer is sync with generator. Then try again."
            )
            exit()
        config_f.close()
...
def trace_get(file_name):
    ...
    with open(file_name, "a+") as file:
        file.write(
            f"{(start_freq + max_x * step_size):.1f}"
