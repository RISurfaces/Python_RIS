...
def check_RIS_metadata(RIS_list: list, client: mqtt_client):
    for ris in RIS_list:
        ...
        if pattern != None:
            val = pattern[3:-1]
            client.publish(topic_params, f"Pattern: {val}")


def set_pattern_with_ack(RIS_list: list, client: mqtt_client, val: str, val2: str):
    for ris in RIS_list:
        ris.set_pattern(val)
        time.sleep(0.1)
        # pattern = ris.read_pattern()
        ...
        # if pattern != None:
        # val_pat = pattern[3:-1]
        client.publish(topic_pattern, f"{val2}")
...
def event_handler(command: str, RIS_list: list, client: mqtt_client):
    if command == "?Params":
        ...
        val2 = command
        val = "".join(("0x", command))
        set_pattern_with_ack(RIS_list, client, val, val2)
