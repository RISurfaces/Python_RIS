...
def hex_to_bin(pattern: str, rows: int, columns: int) -> list:
...
def create_matrix(bin_str: str, rows: int, columns: int):
    ris_matrix = []
    i = 0
...
def A_coeff(
    bin_str: str, A_coeff_min: float, A_coeff_max: float, rows: int, columns: int
) -> int:
